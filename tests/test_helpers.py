...
class TestHelpers(unittest.TestCase):
    ...
    def test_system(self):
        ...
        anadama2.helpers.system(["touch", f])(None)
        self.assertTrue(os.path.isfile(f))
        anadama2.helpers.system(["rm", f])(None)
        self.assertFalse(os.path.isfile(f))
        anadama2.helpers.system(["echo", "hi"], stdout=f)(None)
        s = os.stat(f).st_size
        self.assertGreater(s, 0)
        anadama2.helpers.system(["echo", "hi"], stdout=f)(None)
        self.assertEqual(os.stat(f).st_size, 2*s)
        anadama2.helpers.system(["echo", "hi"], stdout_clobber=f)(None)
        self.assertEqual(os.stat(f).st_size, s)
        e = os.path.join(self.workdir, "err.txt")
        anadama2.helpers.system(["python", '-c' 'import sys; sys.stderr.write("foobaz");'],
                               stderr=e)(None)
        s = os.stat(e).st_size
        self.assertGreater(s, 0)
        anadama2.helpers.system(["python", '-c' 'import sys; sys.stderr.write("foobaz");'],
        ...
        anadama2.helpers.system(["python", '-c' 'import sys; sys.stderr.write(sys.stdin.read());'],
